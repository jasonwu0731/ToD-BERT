        ...
        for dial_dict in dials:
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                for slot in SLOTS:
                    if slot in belief_dict.keys():
                        ...
                        if (
                            ...
                            and "the {}".format(belief_dict[slot]) in ontology[slot].keys()
                        ):
                ...
                if turn["turn_idx"] == 0 and turn["system_transcript"] == "":
                    ...
                    dialog_act[dial_dict["dialogue_idx"].replace(".json", "")][str(turn["turn_idx"])]
                    == "No Annotation"
                ):
                    cur_sys_acts = set()
                else:
...
def prepare_data_multiwoz(args):
    ...
    path_to_ontology_mapping = os.path.join(
        args["data_path"], "MultiWOZ-{}/ontology-mapping{}.json".format(version, args["ontology_version"])
    )
    ...
    dialog_act = json.load(
        open(os.path.join(args["data_path"], "MultiWOZ-{}/dialogue_acts.json".format(version)), "r")
    )

    _example_type = "dial" if "dial" in example_type else example_type

    pair_trn, ontology_mapping, resp_cand_trn = globals()["read_langs_{}".format(_example_type)](
        args, file_trn, ontology_mapping, dialog_act, max_line, args["domain_act"], update_ont_flag
    )

    pair_dev, ontology_mapping, resp_cand_dev = globals()["read_langs_{}".format(_example_type)](
        args, file_dev, ontology_mapping, dialog_act, max_line, args["domain_act"], update_ont_flag
    )

    pair_tst, ontology_mapping, resp_cand_tst = globals()["read_langs_{}".format(_example_type)](
        args, file_tst, ontology_mapping, dialog_act, max_line, args["domain_act"], update_ont_flag
    )
